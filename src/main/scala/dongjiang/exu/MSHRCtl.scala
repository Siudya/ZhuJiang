...
class MSHRCtl()(implicit p: Parameters) extends DJModule {
  ...
  // mshrTable
  val mshrTableReg = RegInit(VecInit(Seq.fill(djparam.nrMSHRSets) {
    VecInit(Seq.fill(djparam.nrMSHRWays) {
      0.U.asTypeOf(new MSHREntry())
    })
  }))
  val mshrLockVecReg = RegInit(VecInit(Seq.fill(djparam.nrMSHRSets) {
    false.B
  })) // TODO: nrMSHRSets -> nrDirSet
   ...
   */
  val respSendSet = RREncoder(respWillSendVecVec.map(_.reduce(_ | _)));
  dontTouch(respSendSet)
  val respSendWay = RREncoder(respWillSendVecVec(respSendSet));
  dontTouch(respSendWay)
   ...
   */
  val reqSendSet = RREncoder(reqWillSendVecVec.map(_.reduce(_ | _)));
  dontTouch(reqSendSet)
  val reqSendWay = RREncoder(reqWillSendVecVec(reqSendSet));
  dontTouch(reqSendWay)
  ...
  // ---------------------------  Assertion  -------------------------------- //
  // MSHR Timeout Check
  val cntMSHRReg = RegInit(VecInit(Seq.fill(djparam.nrMSHRSets) {
    VecInit(Seq.fill(djparam.nrMSHRWays) {
      0.U(64.W)
    })
  }))
  ...
  val cntLockReg = RegInit(VecInit(Seq.fill(djparam.nrMSHRSets) {
    0.U(64.W)
  }))
