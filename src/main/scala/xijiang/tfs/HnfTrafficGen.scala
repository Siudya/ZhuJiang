...
import xijiang.Node
import xijiang.router.{HnfRx, HnfTx}
import zhujiang.chi.ChannelEncodings
import zhujiang.ZJModule

class HnfTrafficGen(local: Boolean, node: Node)(implicit p: Parameters) extends ZJModule {
  val io = IO(new Bundle {
    val rx = Flipped(new HnfTx(node))
    val tx = Flipped(new HnfRx(local, node))
